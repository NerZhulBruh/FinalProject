...
def update_graphs(selected_categories, selected_years):
    ...
    numeric_columns = filtered_df2.select_dtypes(include=['float64', 'int64']).columns
    correlation_heatmap = go.Figure(data=go.Heatmap(
        z=filtered_df2[numeric_columns].corr(),
        x=numeric_columns,
        y=numeric_columns,
        colorscale='Viridis'
    ))
    ...
    return scatter_fig, bar_fig, director_rating_fig, censor_rating_fig, genre_pie_chart, movie_trends_line_chart_df, movie_trends_line_chart_df2,correlation_heatmap
